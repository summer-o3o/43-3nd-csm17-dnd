# [csm17] 수학선생님들을 위한 문제은행 서비스

> 본 프로젝트는 2023년 4월 csm17에서 진행한 기업협업 프로젝트입니다
<br />

<br /><br />

# 📅 개발 기간 및 인원

- 개발 기간: 2023.4.10 ~ 2023.4.04 (4주)
- 개발 인원:
  - FrontEnd : 김수미
  <br /><br /><br />
  
  
# ❶ 배경

## 왜 처음 "수학비서"를 참고하여 기획했나요?<br />
"수학비서"를 선택한 이유는 드래그 앤 드롭의 기능이 무궁무진하게 다양하게 적용될 수 있는 서비스이기 때문입니다.<br>
드래그 앤 드롭은 사용자들이 쉽고 직관적으로 요소들을 이동하고 조작할 수 있는 인터랙션 기술로,<br>
학습자들에게 직관적인 학습 경험을 제공하기 때문에 선택하게 되었습니다.<br><br>

---

<br><br>

# ❷ 서비스 소개

## csm17의 수학비서는, 어떤 서비스인가요?<br />
수학선생님들이 손쉽게 수학문제를 검색해보고 필요한 문제들만 쏙쏙 뽑아서 사용할수 있는 서비스예요!<br />
드래그앤드롭을이용해서 윈도우나 맥을 사용했던것처럼 폴더 합치기, 폴더 이동, 문제이동등 드래그앤드롭이 csm17의 수학비서의 포인트예요!<br /><br /><br />


## soleMates, 어떤 기능이 있나요?
- 로그인, 회원가입
- 메인, 헤더, 푸터
- 문제 리스트
- 문제 필터
- 트리구조
<br><br>


## 로그인, 회원가입
이미지 넣을 자리

- 설명 자리
- 설명 자리
- 설명 자리
- 설명 자리
<br><br>

## 메인, 헤더, 푸터
이미지 넣을 자리

- 설명 자리
- 설명 자리
- 설명 자리
- 설명 자리
<br><br>

## 상품 리스트
이미지 넣을 자리

- 설명 자리
- 설명 자리
- 설명 자리
- 설명 자리
<br><br>

## 상품 상세페이지
이미지 넣을 자리

- 설명 자리
- 설명 자리
- 설명 자리
- 설명 자리
<br><br>

## 장바구니
이미지 넣을 자리

- 설명 자리
- 설명 자리
- 설명 자리
- 설명 자리
<br><br>

---

<br><br>

# ❸ 핵심기술
## 수학비서에서는 어떤 기술을 사용하였을까요?
> react-beautiful-dnd 라이브러리를 사용했어요!
> <br>

- nesting과 extend을 사용하여 css 속성들을 묶어서 재사용이 가능하게 했어요. <br><br>

> React의 hook을 사용했어요!
> <br>

- React의 hook중에 useMemo를 이용해서 랜더링 최적화를 했어요!<br><br>

---

<br><br>

# ❹ 회고
## 수학비서 프로젝트를 진행하면서 프로젝트를 진행하면서 배운점이 있나요?

> 작성중
> <br>

- 작성중
- 작성중
- 작성중<br><br>

> 작성중
> <br>

- 작성중
- 작성중<br><br>

> 작성중
> <br>

- 작성중<br><br>


## soleMates 프로젝트를 진행하면서 부족한점이 있었있나요?

> 작성중
> <br>

- 작성중
- 작성중br><br>


> 작성중
> <br>

- 작성중
- 작성중<br><br>


## soleMates 프로젝트를 진행하면서 개선하고 싶은점이 있었있나요?

> 작성중
> <br>

- 작성중
- 작성중<br><br>

> 작성중
> <br>

- 작성중
- 작성중<br><br>

## soleMates 프로젝트를 진행하면서 느낀점이 있었있나요?

> 작성중
> <br>

- 작성중
- 작성중








